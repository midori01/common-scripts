...
check_system() { [[ $EUID != 0 ]] && { echo "请以 root 权限运行脚本"; exit 1; }; grep -qiE "debian|ubuntu" /etc/issue || { echo "仅支持 Debian 或 Ubuntu 系统"; exit 1; }; arch=$(uname -m); [[ "$arch" =~ ^(x86_64|aarch64)$ ]] || { echo "架构不支持"; exit 1; }; deps=(curl wget xz-utils openssl jq); for dep in "${deps[@]}"; do command -v "$dep" &> /dev/null || missing_deps+=("$dep"); done; [[ ${#missing_deps[@]} -ne 0 ]] && apt install -y "${missing_deps[@]}"; repo_version="shadowsocks"; }
...
case "$1" in
    ...
    obfs) wget -qO /usr/local/bin/obfs-server "https://github.com/midori01/simple-obfs/releases/download/v1.0.2-midori-build/obfs-server-${arch}" && chmod +x /usr/local/bin/obfs-server && echo "obfs-server 安装完成" && modify_json || echo "obfs-server 安装失败" ;;
