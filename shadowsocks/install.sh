...
download_ss() { arch=$(uname -m); version=$(curl -s https://api.github.com/repos/shadowsocks/shadowsocks-rust/releases/latest | jq -r '.tag_name'); wget -qO- "https://github.com/shadowsocks/shadowsocks-rust/releases/download/${version}/shadowsocks-${version}.${arch}-unknown-linux-gnu.tar.xz" | tar -xJ -C /tmp && mv /tmp/ssserver /usr/local/bin/ssserver && chmod +x /usr/local/bin/ssserver && rm -f /tmp/ss* && echo "$version" || { echo "下载或解压失败"; return 1; }; }
