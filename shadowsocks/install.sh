...
case "$1" in
    ...
    uninstall) systemctl disable --now ss-rust > /dev/null 2>&1 && rm -f /usr/local/bin/ss-rust /etc/ss-rust.json /etc/systemd/system/ss-rust.service && systemctl daemon-reload && echo "Shadowsocks Rust 卸载完成" ;;
    ...
    *) set_variable && download_ss && create_json && systemctl enable --now ss-rust > /dev/null 2>&1 && systemctl restart ss-rust && echo "安装完成，端口: ${server_port}, 加密: ${method}, 密码: ${password}" ;;
esac